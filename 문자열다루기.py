...
b = a[0] + a[1] + a[2] + a[3]
print(b)
# a[1] = "L"  # 문자열의 요소는 읽을 수 있지만 변경은 되지않음. 리터럴 상수
...
print(input_str2.count("lo"))   # 문자열에서 해당 문자 혹은 문자열의 갯수를 반환
print(len(input_str2))  # 문자열의 길이를 반환, 별도의 외부 함수를 사용
print(input_str2.__len__()) # 문자열에 포함된 내장 함수

# 문자열 찾기 : find(), rfind(). index()
# find() : 찾은 문자열의 첫 번째 인덱스를 반환, 못찾으면 -1을 반환
# index() : 찾은 문자열의 첫 번째 인덱스를 반환, 못 찾으면 valueError 에외 발생
